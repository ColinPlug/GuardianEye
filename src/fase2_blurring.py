...
def blur_img(image_path, civilian_targets, output_dir="data/veilige_beelden"):
    ...
    for target in civilian_targets:
        ...
        if pts.size == 8:
            ...
            # Shift de coördinaten van het civiele object naar de lokale coördinaten van de ROI
...
def visualize_results(image_path, military_targets, civilian_targets, output_dir="data/inspectie_beelden"):
    ...
    # Visualiseer civiele doelen in blauw
    for target in civilian_targets:
        pts = np.array(target["coords"], np.int32)
    ...
    filename = os.path.basename(image_path)
    output_path = os.path.join(output_dir, f"inspectie_{filename}")
...
if __name__ == "__main__":
    ...
    if os.path.exists(TEST_IMAGE):
        ...
        # Stap 1: Scan de afbeelding om militaire en civiele doelen te identificeren
        military, civilians = scan_image(TEST_IMAGE)

        # Stap 2: Anonimiseren
        print(f"\nToepassen van Privacy Filter op {len(civilians)} civiele doelen...")
        veilige_image_path = blur_img(TEST_IMAGE, civilians)
        print(f"Veilige afbeelding opgeslagen in: {veilige_image_path}")

        # Stap 3: Visuele inspectie van de resultaten
        visualize_results(veilige_image_path, military, civilians)
